...
const RestaurantCard = ({ restaurant, onSelect, isActive, selectedState }) => {
  const dishes = restaurant.dishes || restaurant.menu || [];
  const stateMatch =
    Boolean(selectedState) &&
    ((restaurant.featuredStates || []).includes(selectedState) ||
      dishes.some((dish) => dish.originState === selectedState));
      ...
      <div className="grid gap-3 p-3 sm:grid-cols-[100px_1fr]">
        ...
        <div className="min-w-0 flex flex-col">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {stateMatch && (
              <span className="rounded-full border border-orange-200/80 bg-gradient-to-r from-orange-100 to-amber-100 px-2 py-0.5 text-[11px] font-semibold text-orange-700 dark:border-orange-400/30 dark:from-orange-500/15 dark:to-amber-400/10 dark:text-orange-200">
                {selectedState} spotlight
              </span>
            )}
